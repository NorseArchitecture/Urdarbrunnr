...
- `Norse.Persistence.EntityFramework.Design` — `EfMigrationContributor<TContext>` and `MigrationConnectionStringAttribute`, the EF-specific base that realm `.Migrations` projects implement. `DdlEmittingMigrationsScaffolder` and `AddNorseDesignTimeServices()` landed in the same pass (2026-07-22), provider-agnostic — see the design doc for the mechanism.
- `Norse.Persistence.EntityFramework.Design.PostgreSQL` (+ its `.Generator` sibling) — ships this realm's first Roslyn `IIncrementalGenerator`, which discovers every `EfMigrationContributor<TContext>` **and every `ISeedContributor`** visible in a migrations service's compilation and emits `AddNorseMigrations()`, wiring `AddNorseMigrationsRunner()` and `AddNorseSeedingRunner()` (both Midgard, `Norse.Infrastructure.Migrations`) in the same generated call. It walks **compiled assembly symbols**, never source syntax trees, by design — the plan's verification gate proved identical output whether contributor packages arrive as `ProjectReference` (Bifröst dev mode, today) or `PackageReference` (NuGet/CI mode, tomorrow) before calling the task done. `Design.PostgreSQL`/`Design.SqlServer` also each ship an `IDesignTimeDbContextFactory<TContext>` base (`NorsePostgreSqlDesignTimeDbContextFactory<T>` / `NorseSqlServerDesignTimeDbContextFactory<T>`) that a downstream realm's own factory derives from.

Snake_case naming, previously provided by the external `EFCore.NamingConventions` package, is now implemented in-house via `NorseSnakeCaseNamingConvention` and `SnakeCaseNameRewriter` in `Norse.Persistence.EntityFramework`; the dependency has been removed. The registration API — `useSnakeCaseNaming` parameter defaults and override mechanics on `AddNorsePostgresContext` and `AddNorseSqlServerContext` — remains unchanged. Hardened 2026-07-23 against two real bugs found downstream in Mímisbrunnr, both permanent and load-bearing for every future Postgres+snake_case realm: the EF migrations-history table is now excluded from renaming entirely (its raw-SQL-facing `HistoryRepository.TableName` never went through this convention, so renaming it desynced `LOCK TABLE`/`CREATE TABLE` and 42P01'd on first migrate), and only a JSON-mapped entity's own **root** container column is renamed — a nested JSON entity sharing that container used to get renamed too, which crashes EF Core 11 preview6's JSON query shaper (confirmed as the identical, already-fixed-elsewhere bug behind `dotnet/efcore#37417`). Full design: `../Glitnir/docs/Urdarbrunnr/specs/2026-07-22-inhouse-snake-case-naming-convention-design.md` (see its 2026-07-23 addendum).
