	...
	extension(DbContextOptionsBuilder optionsBuilder)
	{
			...
			Action<IConventionEntityType, Func<string, string>>? applyProviderSpecificRenames = null)
		{
			((IDbContextOptionsBuilderInfrastructure)optionsBuilder)
				.AddOrUpdateExtension(
					new NorseSnakeCaseNamingOptionsExtension(rewriteName, applyProviderSpecificRenames));
			...
			string connectionString, string? migrationsAssemblyName)
		{
			...
			if (provider.TemporalRealizationHook is not null)
				((IDbContextOptionsBuilderInfrastructure)optionsBuilder)
					.AddOrUpdateExtension(
						new NorseTemporalRealizationOptionsExtension(provider.TemporalRealizationHook));
			return optionsBuilder;
		}
	}
}
